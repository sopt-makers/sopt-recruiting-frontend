...
const Content = ({ pass }: { pass?: boolean }) => {
  ...
  const {
    recruitingInfo: { name, group, soptName, season, applicationResultStart, interviewStart },
  } = useRecruitingInfo();
    ...
    <>
      {pass ? (
        <p className={contentVar[deviceType]}>
          ...
          {__IS_MAKERS__ ? (
            ...
            `}
              ...
              <span className={strongText[__IS_MAKERS__ ? 'makers' : 'sopt']}>{'2월 21일 ~ 2월 22일'}</span>
              <span className="amp-mask">{` 기간 중 진행될 예정입니다.\n\n`}</span>

              <span className="amp-mask">{`원활한 면접 집행을 위해, 아래 구글폼에 `}</span>
              <span className={strongText[__IS_MAKERS__ ? 'makers' : 'sopt']}>{'불가능한 시간대를 모두 선택'}</span>
              <span className="amp-mask">{`해 제출 부탁드립니다.\n`}</span>

              <span className="amp-mask">{`( 제출 마감 : `}</span>
              <span className={strongText[__IS_MAKERS__ ? 'makers' : 'sopt']}>{'2월 18일 수요일 오후 8시 '}</span>
              <span className="amp-mask">{`)\n`}</span>
              ...
              <span>
                {`
            제출해주신 폼 제출 내용을 기반으로 확정된 면접 일정 및 면접 안내를 내일(2월 19일) 오후 2시 전으로 이메일을 통해 공유드리겠습니다.

            다시 한 번 진심으로 축하드리며, 인터뷰에서 뵙겠습니다!

            문의사항 및 궁금한 점은 아래 연락처로 문의 부탁드립니다.
            김성민 : 010-6503-2548
            ...
            `}
              ...
              <span className={strongText[__IS_MAKERS__ ? 'makers' : 'sopt']}>
                {`${formattedInterviewStartWithDay} ~ 3/1 (일) `} 양일 간
              </span>
              <span className="amp-mask">{`\n온라인으로 진행될 예정입니다.\n\n`}</span>

              <span className="amp-mask">{`모든 면접 대상자 분들을 대상으로 면접 가능 시간을 조사하려 합니다.\n아래 구글폼을 내일 20시 (2월 26일 목요일 오후 8시) 까지 제출해주세요.\n\n`}</span>
            ...
            내일 모레 정오 (27일 금요일 오후 12시) 전으로 이메일을 통해 전해드리겠습니다.
        ...
        <p className={`amp-mask ${contentVar[deviceType]}`} style={{ wordBreak: 'keep-all' }}>
          ...
          SOPT makers에 귀한 시간을 내어주셔서 다시 한 번 깊이 감사드립니다 :)
